...
/**
 * Heroカードの中身。Before/Afterスライダーで2回描画されるため、
 * 1回分のマークアップをここに切り出している。
 * (Before側は .hero-before スコープのCSSで「古いページ風」に変装する)
 */
function HeroInner({ locale }: { locale: Locale }) {
  const t = getMessages(locale).hero;

  return (
    <div className="relative px-5 py-6 sm:px-6 sm:py-7 md:px-10 md:py-9 lg:px-14 lg:py-10">
      {/* subtle grid */}
      <div
        className="hero-decor pointer-events-none absolute inset-0"
        style={{
          backgroundImage: `
      ...
      {/* corner glow */}
      <div
        className="hero-decor pointer-events-none absolute -right-32 -top-32 h-[480px] w-[480px] rounded-full"
        style={{
          background:
      ...
      {/* top rule */}
      <div
        className="hero-decor pointer-events-none absolute inset-x-0 top-0 h-px"
        style={{
          background:
...
export default function Hero({ locale }: { locale: Locale }) {
  ...
  return (
    <SectionShell className="pt-1 sm:pt-2">
      <Card gradientBorder className="relative overflow-hidden rounded-3xl p-0">
        <HeroBeforeAfter
          labels={t.beforeAfter}
          before={<HeroInner locale={locale} />}
          after={<HeroInner locale={locale} />}
        />
      </Card>
    </SectionShell>
  );
